function errorCondition(errorCode) {
    if (errorCode == 0) {
        ...
        errorCode = "Function dataHold called with incorrect dataRW and/or pointStr.";
    }
    else {
...
// This function is how we will store and interact with data.
//    uses the localStorage function to read/write data
function dataHold(dataRW, pointStr) {
    if (dataRW == undefined && pointStr == undefined) {
        localStorage.setItem(tableValue, 0)
        localStorage.setItem(lastChange, 0);
        localStorage.setItem(tableItemList, "");
        localStorage.setItem(tableItemIndex, 0);
        localStorage.setItem(undoAllow, false);
    ...
    if (pointStr == "tableValue") {
        if (dataRW != NaN) {
            localStorage.setItem(tableValue, dataRW)
        }
        else {
            dataRW = localStorage.getItem(tableValue);
        }
    }
    else if (pointStr == "lastChange") {
        if (dataRW != NaN) {
            localStorage.setItem(lastChange, dataRW);
        }
        else {
            dataRW = dataRW = localStorage.getItem(lastChange);
        }
    }
    else if (pointStr == "tableItemList") {
        if (dataRW != NaN) {
            localStorage.setItem(tableItemIndex, dataRW);
        }
        else {
            dataRW = dataRW = localStorage.getItem(tableItemList);
        }
    }
    else if (pointStr == "tableItemIndex") {
        if (dataRW != NaN) {
            localStorage.setItem(tableItemIndex, dataRW);
        }
        else {
            dataRW = dataRW = localStorage.getItem(tableItemIndex);
        }
    }
    else if (pointStr == "undoAllow") {
        if (dataRW != NaN) {
            localStorage.setItem(undoAllow, dataRW)
        }
        else {
            dataRW = dataRW = localStorage.getItem(undoAllow);
        }
    }
    else if (pointStr == "userInput") {
        if (dataRW != NaN) {
            localStorage.setItem(userInput, dataRW)
        }
        else {
            dataRW = dataRW = localStorage.getItem(userInput);
        }
    }
    else if (isNaN(dataRW) == isNaN(pointStr)) {
        // pass
    }
    else {
        errorCondition(8);
...
function resetAll() {
    ...
    tableValue = dataHold(NaN, "tableValue")
    lastChange = dataHold(NaN, "lastChange")
    tableItemList = dataHold(NaN, "tableItemList")
